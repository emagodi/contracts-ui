...
export default function ProcurementApprovalViewPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setItem(data || null);
        setProcComments(String(data?.procurementComments || ""));
      } catch (e: unknown) {
  ...
  const confirmSubmit = async () => {
    ...
    try {
      const status = decision === "APPROVE" ? "PROCUREMENT_APPROVED" : "PROCUREMENT_REJECTED";
      const updateBody: Record<string, unknown> = {
        approvalStatus: status,
        procurementSignatureDate: new Date().toISOString(),
      ...
      router.push("/requisitions/procurement-approvals");
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : String(err);
      setConfirmError(msg);
    } finally {
    ...
    <div className="max-w-5xl mx-auto mt-8 bg-white border-2 border-black shadow-lg rounded-sm p-10 text-gray-900">
      ...
      <form onSubmit={handleSubmit} className="space-y-6 text-black">
        ...
        <div className="border border-black p-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            {commercialPreviewUrl ? (
              ...
              <Input readOnly value={String(item?.commercialSignature || "").startsWith("/") ? "" : (item?.commercialSignature || "")} className="border border-gray-400 text-gray-500 bg-gray-200 rounded-md" />
            )}
            <span className="border border-gray-400 text-gray-500 bg-gray-200 p-2 rounded-md">{formatDisplayDate(item?.commercialSignatureDate)}</span>
          </div>
          <TextArea readOnly value={item?.commercialComments || ""} className="mt-3 border border-gray-400 text-gray-500 bg-gray-200 rounded-md" />
        </div>
        ...
        <div className="border border-black p-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="flex items-center gap-3">
              {signaturePreviewUrl ? (
                ...
                <Button size="sm" className="w-full h-12 bg-emerald-600 hover:bg-emerald-700 text-white rounded-md" onClick={signProcurement}>Sign</Button>
              )}
          ...
          <TextArea value={procComments} onChange={(e) => setProcComments(e.target.value)} placeholder="Procurement Comments" className="mt-3 border border-gray-400 text-black bg-white rounded-md" />
        </div>
      ...
      <Modal isOpen={confirmOpen} onClose={() => setConfirmOpen(false)} className="max-w-[560px] p-6 lg:p-10">
        <div>
          <h3 className="text-lg font-semibold mb-4 text-sky-800">Confirm Procurement Approval</h3>
