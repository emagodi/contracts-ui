...
type NavItem = {
  ...
  path?: string;
  roles?: string[];
  subItems?: { name: string; path: string; roles?: string[]; pro?: boolean; new?: boolean }[];
};
...
const navItems: NavItem[] = [
  {
    icon: <GridIcon />,
    name: "Dashboard",
    subItems: [{ name: "Reports", path: "/", pro: false, roles: ALL_ROLES }],
  },
  {
    icon: <CalenderIcon />,
    name: "Calendar",
...
const AppSidebar: React.FC = () => {
  ...
  const isAllowed = useCallback(
    (allowed?: string[]) =>
      !allowed || allowed.length === 0 || userRoles.length === 0 || allowed.some((role) => userRoles.includes(role)),
    [userRoles]
  );
  ...
  const renderMenuItems = (
    ...
    <ul className="flex flex-col gap-4">
      {navItems
        .filter((nav) =>
          nav.subItems
            ? (nav.subItems || []).some((si) => isAllowed(si.roles))
            : isAllowed(nav.roles)
        )
        .map((nav, index) => (
        <li key={nav.name}>
          ...
          {nav.subItems && (isExpanded || isHovered || isMobileOpen) && (
            ...
            >
              <ul className="mt-2 space-y-1 ml-9">
                {(nav.subItems || [])
                  .filter((subItem) => isAllowed(subItem.roles))
                  .map((subItem) => (
                  <li key={subItem.name}>
                    <Link
                      href={subItem.path}
