...
export default function BusinessApprovalQueuePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const res = await fetch(`/api/approvals/by-status?status=${encodeURIComponent("COMMERCIAL_APPROVED")}`, { headers: authHeaders() });
        if (!res.ok) {
  ...
  return (
    <div>
      <PageBreadcrumb pageTitle="Business Dev Approval Queue" />
      <div className="overflow-hidden rounded-2xl border border-gray-200 bg-gradient-to-br from-white to-gray-50 shadow-theme-xs dark:border-white/[0.05] dark:bg-white/[0.03]">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-white/[0.05]">
          <div>
            <p className="font-semibold text-gray-800 text-theme-sm dark:text-white/90">Approvals Awaiting Business Development Decision</p>
            <p className="text-gray-500 text-theme-xs dark:text-gray-400">Filtered by status COMMERCIAL_APPROVED</p>
          </div>
            ...
            <Table>
              <TableHeader className="border-y border-gray-100 dark:border-white/[0.05] bg-gray-50/80">
                <TableRow>
                  ...
                  <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400">Commercial Comments</TableCell>
                ...
                {pagedItems.map((r) => (
                  <TableRow key={r.id} className="hover:bg-blue-50/50 transition-colors">
                    ...
                    <TableCell className="px-5 py-4 text-start max-w-[380px] truncate">
                      <span title={String(r.commercialComments || "-")}>{String(r.commercialComments || "-")}</span>
                    </TableCell>
                    <TableCell className="px-5 py-4 text-start">
                      <Button size="xs" variant="outline" startIcon={<EyeIcon />} className="rounded-full text-sky-700 ring-1 ring-inset ring-sky-200 hover:bg-sky-50" onClick={() => router.push(`/requisitions/business-approvals/${r.id}`)}>View</Button>
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
