...
export default function CommercialApprovalViewPage() {
  ...
  useEffect(() => {
    ...
    load(item?.financialSignature, setFinancialPreviewUrl);
  ...
  const confirmSubmit = async () => {
    ...
    try {
      const status = decision === "APPROVE" ? "COMMERCIAL_APPROVED" : "COMMERCIAL_REJECTED";
      const updateBody: Record<string, unknown> = {
        approvalStatus: status,
        commercialSignatureDate: new Date().toISOString(),
      };
      if (signaturePath) updateBody.commercialSignature = signaturePath;
      ...
      router.push("/requisitions/commercial-approvals");
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : String(err);
      setConfirmError(msg);
    } finally {
    ...
    <div className="max-w-5xl mx-auto mt-8 bg-white border-2 border-black shadow-lg rounded-sm p-10 text-gray-900">
        ...
        <div className="p-6 text-sm text-black space-y-3">
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 mr-2">
              <Input readOnly value={item?.approvalTo || ""} className="border border-gray-400 text-gray-500 bg-gray-200 rounded-md" />
            </div>
          </div>
          <div className="flex items-center gap-4">
            <span className="font-semibold w-20">Date:</span>
            <span className="flex-1 mr-2 border border-gray-400 text-black bg-white p-2 rounded-md">{currentDateDisplay}</span>
          </div>
          <div className="flex items-start gap-4">
            <span className="font-semibold w-20">RE:</span>
            <div className="flex-1 mr-2">
              <TextArea readOnly value={item?.approvalReference || item?.approvalSubject || ""} className="border border-gray-400 text-gray-500 bg-gray-200 rounded-md" rows={4} />
            </div>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 text-black">
        <div className="border border-black p-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            {legalPreviewUrl ? (
              ...
              <Input readOnly value={String(item?.legalSignature || "").startsWith("/") ? "" : (item?.legalSignature || "")} className="border border-gray-400 text-gray-500 bg-gray-200 rounded-md" />
            )}
            <span className="border border-gray-400 text-gray-500 bg-gray-200 p-2 rounded-md">{formatDisplayDate(item?.legalSignatureDate)}</span>
          </div>
          <TextArea readOnly value={item?.legalComments || ""} className="mt-3 border border-gray-400 text-gray-500 bg-gray-200 rounded-md" />
        </div>
        ...
        <div className="border border-black p-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            {technicalPreviewUrl ? (
              ...
              <Input readOnly value={String(item?.technicalSignature || "").startsWith("/") ? "" : (item?.technicalSignature || "")} className="border border-gray-400 text-gray-500 bg-gray-200 rounded-md" />
            )}
            <span className="border border-gray-400 text-gray-500 bg-gray-200 p-2 rounded-md">{formatDisplayDate(item?.technicalSignatureDate)}</span>
          </div>
          <TextArea readOnly value={item?.technicalComments || ""} className="mt-3 border border-gray-400 text-gray-500 bg-gray-200 rounded-md" />
        </div>
        ...
        <div className="border border-black p-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            {financialPreviewUrl ? (
              ...
              <Input readOnly value={String(item?.financialSignature || "").startsWith("/") ? "" : (item?.financialSignature || "")} className="border border-gray-400 text-gray-500 bg-gray-200 rounded-md" />
            )}
            <span className="border border-gray-400 text-gray-500 bg-gray-200 p-2 rounded-md">{formatDisplayDate(item?.financialSignatureDate)}</span>
          </div>
          <TextArea readOnly value={item?.financialComments || ""} className="mt-3 border border-gray-400 text-gray-500 bg-gray-200 rounded-md" />
        </div>
        ...
        <div className="border border-black p-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            {bdPreviewUrl ? (
              ...
              <Input readOnly value={String(item?.businessDevelopmentSignature || "").startsWith("/") ? "" : (item?.businessDevelopmentSignature || "")} className="border border-gray-400 text-gray-500 bg-gray-200 rounded-md" />
            )}
            <span className="border border-gray-400 text-gray-500 bg-gray-200 p-2 rounded-md">{formatDisplayDate(item?.businessDevelopmentSignatureDate)}</span>
          </div>
          <TextArea readOnly value={item?.businessDevelopmentComments || ""} className="mt-3 border border-gray-400 text-gray-500 bg-gray-200 rounded-md" />
        </div>
        ...
        <div className="border border-black p-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            {procPreviewUrl ? (
              ...
              <Input readOnly value={String(item?.procurementSignature || "").startsWith("/") ? "" : (item?.procurementSignature || "")} className="border border-gray-400 text-gray-500 bg-gray-200 rounded-md" />
            )}
            <span className="border border-gray-400 text-gray-500 bg-gray-200 p-2 rounded-md">{formatDisplayDate(item?.procurementSignatureDate)}</span>
          </div>
          <TextArea readOnly value={item?.procurementComments || ""} className="mt-3 border border-gray-400 text-gray-500 bg-gray-200 rounded-md" />
        </div>
        ...
        <Button type="submit" className="w-full bg-blue-900 hover:bg-blue-700 text-white font-semibold mt-6">
          Submit Commercial Decision
        </Button>
      </form>

      <Modal isOpen={confirmOpen} onClose={() => setConfirmOpen(false)} className="max-w-[560px] p-6 lg:p-10">
        <div>
          <h3 className="text-lg font-semibold mb-4 text-sky-800">Confirm Commercial Approval</h3>
          <p className="text-sm text-gray-700 mb-4">Decision: {decision === "APPROVE" ? "APPROVE" : "REJECT"}</p>
