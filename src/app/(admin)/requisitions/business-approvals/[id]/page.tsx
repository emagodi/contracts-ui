...
export default function BusinessApprovalViewPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setItem(data || null);
        setBdComments(String(data?.businessDevelopmentComments || ""));
      } catch (e: unknown) {
  ...
  const confirmSubmit = async () => {
    ...
    try {
      const status = decision === "APPROVE" ? "BUSINESS_DEVELOPMENT_APPROVED" : "BUSINESS_DEVELOPMENT_REJECTED";
      const updateBody: Record<string, unknown> = {
        approvalStatus: status,
        businessDevelopmentSignatureDate: new Date().toISOString(),
        businessDevelopmentComments: bdComments,
      };
      if (signaturePath) updateBody.businessDevelopmentSignature = signaturePath;
      ...
      router.push("/requisitions/business-approvals");
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : String(err);
      setConfirmError(msg);
    } finally {
    ...
    <div className="max-w-5xl mx-auto mt-8 bg-white border-2 border-black shadow-lg rounded-sm p-10 text-gray-900">
      ...
      <form onSubmit={handleSubmit} className="space-y-6 text-black">
        ...
        <div className="border border-black p-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="flex items-center gap-3">
              {signaturePreviewUrl ? (
                ...
                <Button size="sm" className="w-full h-12 bg-emerald-600 hover:bg-emerald-700 text-white rounded-md" onClick={signBusiness}>Sign</Button>
              )}
          ...
          <TextArea value={bdComments} onChange={(e) => setBdComments(e.target.value)} placeholder="Business Development Comments" className="mt-3 border border-gray-400 text-black bg-white rounded-md" />
        </div>
        ...
        <Button type="submit" className="w-full bg-blue-900 hover:bg-blue-700 text-white font-semibold mt-6">
          Submit Business Development Decision
        </Button>
      </form>

      <Modal isOpen={confirmOpen} onClose={() => setConfirmOpen(false)} className="max-w-[560px] p-6 lg:p-10">
        <div>
          <h3 className="text-lg font-semibold mb-4 text-sky-800">Confirm Business Development Approval</h3>
