...
import { EyeIcon, DocsIcon, ArrowUpIcon, ArrowRightIcon } from "@/icons";
...
export default function ApprovalQueuePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const url = `/api/requisitions/without-approval`;
  ...
  const triggerUpload = (reqId: number) => {
    const input = document.getElementById(`approvals-file-upload-${reqId}`) as HTMLInputElement | null;
    input?.click();
  };
    ...
    <div>
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-200 bg-gradient-to-br from-white to-gray-50 shadow-theme-xs dark:border-white/[0.05] dark:bg-white/[0.03]">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-white/[0.05]">
          <div>
            <p className="font-semibold text-gray-800 text-theme-sm dark:text-white/90">Requisitions Without Approval</p>
            <p className="text-gray-500 text-theme-xs dark:text-gray-400">Start approval workflow</p>
          </div>
          ...
          <div className="min-w-[980px]">
            <Table>
              ...
              <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                {loading ? (
                  <TableRow><TableCell className="px-5 py-4" colSpan={7}>Loading...</TableCell></TableRow>
                ) : error ? (
                  <TableRow><TableCell className="px-5 py-4 text-red-600" colSpan={7}>{error}</TableCell></TableRow>
                ) : pagedItems.length === 0 ? (
                  <TableRow><TableCell className="px-5 py-4" colSpan={7}>No requisitions</TableCell></TableRow>
                ) : (
                  pagedItems.map((r) => (
                    <TableRow key={r.id} className="hover:bg-blue-50/50 transition-colors">
                      ...
                      <TableCell className="px-5 py-4 text-start">
                        <div className="flex items-center gap-2">
                          <Button size="xs" variant="outline" startIcon={<EyeIcon />} className="rounded-full text-sky-700 ring-1 ring-inset ring-sky-200 hover:bg-sky-50" onClick={() => router.push(`/requisitions/secretary/${r.id}`)}>View</Button>
                          ...
                          <input id={`approvals-file-upload-${r.id}`} type="file" multiple className="hidden" onChange={(e) => uploadFiles(r.id, e.target.files)} />
                          ...
                          <Button size="xs" startIcon={<ArrowRightIcon />} className="rounded-full bg-indigo-600 hover:bg-indigo-700 text-white" onClick={() => router.push(`/requisitions/approvals/${r.id}`)}>Initiate Approval</Button>
                        </div>
                      </TableCell>
                    </TableRow>
                  ))
          ...
          <div className="max-w-full overflow-x-auto">
            <div className="min-w-[700px]">
              <Table>
                ...
                <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                  {attachmentsLoading ? (
                    <TableRow><TableCell className="px-5 py-4" colSpan={4}>Loading...</TableCell></TableRow>
                  ) : attachments.length === 0 ? (
                    <TableRow><TableCell className="px-5 py-4" colSpan={4}>No attachments</TableCell></TableRow>
                  ) : (
                    attachments.map((a) => (
