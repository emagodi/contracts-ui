...
export default function ProcurementManagerQueuePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const url = `/api/requisitions/by-status?status=FINANCEDIRECTOR_APPROVED`;
  ...
  const triggerUpload = (reqId: number) => {
    const input = document.getElementById(`pm-file-upload-${reqId}`) as HTMLInputElement | null;
    input?.click();
  };
  ...
  return (
    <div>
      <PageBreadcrumb pageTitle="Procurement Manager Queue" />
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-200 bg-gradient-to-br from-white to-gray-50 shadow-theme-xs dark:border-white/[0.05] dark:bg-white/[0.03]">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-white/[0.05]">
          <div>
            <p className="font-semibold text-gray-800 text-theme-sm dark:text-white/90">FINANCEDIRECTOR_APPROVED Requisitions</p>
            <p className="text-gray-500 text-theme-xs dark:text-gray-400">For Procurement Manager review</p>
          </div>
            ...
            <Table>
              ...
              <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                {loading ? (
                  ...
                  <TableRow>
                    <TableCell className="px-5 py-4" colSpan={7}>No requisitions</TableCell>
                  </TableRow>
                ) : (
                  pagedItems.map((r) => (
                    <TableRow key={r.id} className="hover:bg-blue-50/50 transition-colors">
                      ...
                      <TableCell className="px-5 py-4 text-start">
                        <div className="flex items-center gap-2">
                          <Button size="xs" variant="outline" startIcon={<EyeIcon />} className="rounded-full text-sky-700 ring-1 ring-inset ring-sky-200 hover:bg-sky-50" onClick={() => router.push(`/requisitions/procurement/${r.id}`)}><span className="sr-only">View</span></Button>
                          ...
                          <input id={`pm-file-upload-${r.id}`} type="file" multiple className="hidden" onChange={(e) => uploadFiles(r.id, e.target.files)} />
